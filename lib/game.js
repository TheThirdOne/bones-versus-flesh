...
c.onmousemove = function (e) {
    ...
    state.mouse.x = (e.layerX - rect.left)|0;
    state.mouse.y = (e.layerY - rect.top)|0;
}
...
function loop(){
    ...
    if (changed || state.mouse.down) {
        changed = false;
        let keys = state.keys;
        replays[0].push([delta, state.mouse.x, state.mouse.y,state.player.x,state.player.y,keys[left],keys[right],keys[jump],keys[reload],state.mouse.down])
    } else {
        replays[0].push([delta, state.mouse.x, state.mouse.y]);
    }
