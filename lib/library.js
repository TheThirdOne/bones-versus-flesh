function onGround(){
	return collide(skeleton.getX(),skeleton.getY()+64)||collide(skeleton.getX() + skeleton.getWidth()*skeleton.getScaleX(),skeleton.getY()+64);
}
function collide(x,y){
	...
	return false;
	/*var temp = stage.getIntersection({'x': x,'y': y});
	if(!temp)
		return false;
	if(temp.pixel[0]==57)
		return false;
	return true; //!(!stage.getIntersection({'x': x,'y': y})); //returns true/false from object or null*/
}
...
var goingLeft = false, goingRight = false, jumped = false, reloading = false, pointdown = false, pointup=false, firing = false;
	...
	player = {
		setX: function (x){
			if(x > stage.getWidth() * .7 || x < stage.getWidth() * .3 ){
				...
				skeleton.setX(x);
				gun.setX(x + ((pointdown)?((skeleton.getScaleX() > 0)?1:-1):0)*32);
			}
		},
		setY: function (y){
			skeleton.setY(y);
			gun.setY(y+25 + ((pointdown)?-8:0) + ((pointup)?8:0));
		},
