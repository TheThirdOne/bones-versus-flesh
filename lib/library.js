...
for (a=0;a<5;a++) {									
	sounds.shotgun[a] = {};
	sounds.shotgun[a]['channel'] = new Audio();		
	sounds.shotgun[a]['channel'].src = document.getElementById('shotgun_sound').src;	
	sounds.shotgun[a]['channel'].load();			
	sounds.shotgun[a]['finished'] = -1;							
}
var temp;
function play_multi_sound(s) {
	for (a=0;a<sounds[s].length;a++) {
		...
		if (sounds[s][a]['finished'] < thistime.getTime()) {			// is this channel finished?
			sounds[s][a]['finished'] = thistime.getTime() + document.getElementById('shotgun_sound').duration*1000;
			/*if(audiochannels[a]['channel'].src != document.getElementById(s).src){
