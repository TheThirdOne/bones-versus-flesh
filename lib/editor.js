...
function render(state) {
    ...
    for (let i in state.replays) {
        ...
        ctx.lineWidth = 1;
        ctx.beginPath();
        ctx.moveTo(state.replays[i][0][0] - camera.x,state.replays[i][1][0] - camera.y)
        for (let j in state.replays[i][0]) {
            ctx.lineTo(state.replays[i][0][j] - camera.x,state.replays[i][1][j] - camera.y)
        }
        ctx.stroke();
    }
}
