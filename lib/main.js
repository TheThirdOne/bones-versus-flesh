...
function new_state() {
    return {
            player: {x:100,y:100,width:32,height:64, 
                    ...
                    ammo: 6, can_fire: true, last_grounded: 0,
...
function update_animation(delta,state, actions) {
    ...
    let runnable = (state.player.last_grounded == 0) && (state.keys[left] != state.keys[right]);
...
function update(delta,state) {
    ...
    for (var block of state.blocks) {
        ...
        if (collides(state.player, block)) {
            ...
            let player = {x:state.player.x, y: state.player.y};
          //  if (amount != 0) console.log("Ejecting player from ",block, player, amount, speed);
            if(min_index < 2) {
    ...
    if (grounded && state.player.last_grounded != 0) {
        actions.push("landed");
    }
    if (grounded) {
    ...
    if ((state.player.last_grounded <= 100 || grounded) && state.keys[jump]) {
        actions.push("jumped");
        state.player.vy = Math.min(state.player.vy,0);
        state.player.vy += C.jump;
        state.player.last_grounded = 10000;
        grounded = false;
    }    
...
//        console.log('Fired weapon: ', state.player.x, state.player.y, state.player.vx,state.player.vy,state.mouse.x,state.mouse.y);
    ...
    if (grounded) {
        state.player.last_grounded = 0;
    } else {
        state.player.last_grounded += delta;
    }
    return actions;
} 
